var VuePicker = (function (exports, vue) {
  ...
  var script$1 = {
    ...
    setup: function setup () {
      var ref$1 = useKeyboardListener();
      var listen = ref$1.listen;
      var unlisten = ref$1.unlisten;
      var registerActions = ref$1.registerActions;
      ...
      var focusOpener = function () { console.log(openerRef); openerRef.value.focus(); };
      var blurOpener = function () { console.log(openerRef); openerRef.value.blur(); };
    ...
    watch: {
      modelValue: function modelValue (nV, oV) { (nV !== oV) && this.selectByValue(this.modelValue); },
    },
    ...
    mounted: function mounted () {
      ...
      this.dropdown.onShow(function () {
        if (this$1.curOpt) { this$1.$nextTick(function () { return this$1.curOpt.focus(); }); }
        else { this$1.blurOpener(); }
        this$1.$emit('open');
      });
      this.dropdown.onHide(function (isOuterClick) {
        if (!isOuterClick) { this$1.focusOpener(); }
  ...
  function render$1(_ctx, _cache, $props, $setup, $data, $options) {
    return (vue.openBlock(), vue.createBlock("div", {
      ...
      vue.createVNode("button", {
        ...
        type: "button",
        onClick: _cache[1] || (_cache[1] = function ($event) { return ($setup.dropdown.toggle($event)); }),
        onKeydown: [
          _cache[2] || (_cache[2] = vue.withKeys(vue.withModifiers(function ($event) { return ($setup.dropdown.toggle($event)); }, ["alt","stop","prevent"]), ["up"])),
          _cache[3] || (_cache[3] = vue.withKeys(vue.withModifiers(function ($event) { return ($options.selectPrev($event)); }, ["exact","stop","prevent"]), ["up"])),
          _cache[4] || (_cache[4] = vue.withKeys(vue.withModifiers(function ($event) { return ($setup.dropdown.toggle($event)); }, ["alt","stop","prevent"]), ["down"])),
          _cache[5] || (_cache[5] = vue.withKeys(vue.withModifiers(function ($event) { return ($options.selectNext($event)); }, ["exact","stop","prevent"]), ["down"])),
          _cache[6] || (_cache[6] = vue.withKeys(vue.withModifiers(function ($event) { return ($options.selectFirst($event)); }, ["stop","prevent"]), ["home"])),
          _cache[7] || (_cache[7] = vue.withKeys(vue.withModifiers(function ($event) { return ($options.selectLast($event)); }, ["stop","prevent"]), ["end"]))
        ],
  ...
  var script = {
    ...
    setup: function setup (props) {
      ...
      picker.regOpt({
        value: value.value,
        optHtml: vue.computed(
          function () { console.log(text.value, (btnRef.value && btnRef.value.innerHTML), value.value); text.value || (btnRef.value && btnRef.value.innerHTML) || value.value; }
        ),
        optTxt: vue.computed(
          function () { console.log(text.value, (btnRef.value && btnRef.value.innerText), value.value); text.value || (btnRef.value && btnRef.value.innerText) || value.value; }
        ),
        setIsSelected: function () { isSelected.value = false; },
